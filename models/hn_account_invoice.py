...
class KsGlobalDiscountInvoice(models.Model):
    ...
    ks_global_discount_type = fields.Selection([
                                                ('percent', 'Percentage'),
                                                ('amount', 'Montant')],
                                               string='Retenue à la source type',
                                               readonly=True,
                                                states={'draft': [('readonly', False)],
                                                        'sent': [('readonly', False)]},
                                                default='percent')
    ks_global_discount_rate = fields.Float('Retenue à la source %' ,
                                           default = lambda self: self.env['ir.config_parameter'].sudo().get_param('hn_amount_rs'),
                                           readonly=True,
                                           states={'draft': [('readonly', False)],
                                                   'sent': [('readonly', False)]})
    ks_amount_discount = fields.Monetary(string='Retenue à la source ',
                                         readonly=True,
                                         compute='_compute_amount',
                                         store=True, track_visibility='always')
    ks_sub_total = fields.Monetary(string='Sous-total',readonly=True)
    ...
    def ks_calculate_discount(self):
        for rec in self:
            if rec.hn_amount_global_tax:
                timbre = rec.hn_amount_global_tax
            else:
                timbre = 0
            ...
            if rec.ks_global_discount_type == "amount":
                ...
                if rec.ks_global_discount_rate != 0.0:
                    ...
                    rec.ks_sub_total  = rec.amount_untaxed + rec.amount_tax + timbre
                else:
    ...
    def ks_update_universal_discount(self):
         for rec in self:
            already_exists = self.line_ids.filtered(
                lambda line: line.name and line.name.find('RS') == 0)
